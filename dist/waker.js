...
var util = require('util'),
    ...
    newSocket = require('./ps4socket'),
    MIN_WAIT_FOR_WAKE = 30000,
    MAX_RETRIES = 5,
...
Waker.prototype.wake = function (detectOpts, device, callback) {
    ...
    if (!(typeof detectOpts === 'number' || typeof callback === 'function' || typeof detectOpts === 'object' && device && (device['host-name'] || typeof device === 'function'))) {
    ...
    var self = this;
    Detector.findAny(detectOpts, null, function (err, device, rinfo) {
...
Waker.prototype._doWake = function (device, detectOpts, callback) {
    ...
    this.readCredentials(function (err, creds) {
        ...
        if (device.status && device.status !== 'Standby' && self.config.errorIfAwake) {
            return callback(new Error(device['host-name'] + ' is already awake! (' + device.status + ')'));
        }
...
Waker.prototype.sendWake = function (device, detectOpts, creds, callback) {
    ...
    this.udp.bind(undefined, detectOpts.bindAddress, function () {
        ...
        self._whenAwake(device, detectOpts, self._login.bind(self, device, creds, detectOpts, callback));
    });
};
    ...
    var loop = function loop(err, d) {
        ...
        if (d.statusLine != ps4lib.STATUS_AWAKE) {
            ...
            if (newTimeout > 0) {
                ...
                setTimeout(function () {
                    Detector.find(device.address, detectOpts, null, loop);
                }, statusCheckDelay);
            } else {
                self.udp.close();
...
Waker.prototype._login = function (device, creds, loginOpts, callback, err) {
    ...
    socket.on('login_result', function (packet) {
        ...
        if (socket.retries++ < MAX_RETRIES && err.code === 'ECONNREFUSED') {
            ...
            self._warn("Login connect refused; retrying soon");
            setTimeout(function () {
        ...
        // TODO as above
        self._error("Error logging in:", err);
