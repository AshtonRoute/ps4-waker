...
Packet.prototype.readString = function (index, length) {
    let end = length === undefined ? this.length() : index + length;
...
class PacketFactory {
    ...
    newByePacket() {
        return this.create(8).writeInt(4); // packet type
    }

    newCHelloPacket() {
        return this.create(28).writeInt(0x6f636370) // packet type
        .writeInt(VERSION).writeInt(0);
    }
    ...
    newHandshakePacket(shello) {
        ...
        return this.create(280).writeInt(32).write(key).write(seed);
    }
    ...
    newOskChangeStringPacket(opts, string) {
        ...
        return this.create(28 + stringLen).writeInt(14) // packet type
        .writeInt(opts.preEditIndex).writeInt(opts.preEditLength).writeInt(opts.caretIndex).writeInt(opts.editIndex).writeInt(opts.editLength).write(stringBuf);
    }

    newOskStartPacket() {
        return this.create(8).writeInt(12); // packet type
    }
    ...
    newOskControlPacket(command) {
        ...
        return this.create(12).writeInt(16) // packet type
        .writeInt(command);
    }

    newStatusPacket(status) {
        return this.create(12).writeInt(20) // packet type
        .writeInt(status || 0);
    }

    newBootRequestPacket(titleId) {
        return this.create(8 + 16).writeInt(10) // packet type
        .write(titleId, 16);
    }
    ...
    newBootRequest2Packet(titleId) {
        ...
        return this.create(92 + 16).writeInt(36) // packet type
        .writeInt(i).write("", 12) // "reserved 1"
        .write("", 64) // "reserved 2"
    ...
    newRemoteControlPacket(op, holdTime) {
        holdTime = holdTime || 0;
        return this.create(16).writeInt(28) // packet type
        .writeInt(op).writeInt(holdTime);
    }
}
